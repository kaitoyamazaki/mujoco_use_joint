...
const double INCREMENT = DEG_TO_RAD / 100;
double joint_targets[2] = {0.0, 0.0};
...
void move_joints() {
    // Set control targets
    d->ctrl[mj_name2id(m, mjOBJ_ACTUATOR, "joint1")] = joint_targets[0];
    d->ctrl[mj_name2id(m, mjOBJ_ACTUATOR, "joint2")] = joint_targets[1];
}
...
void simulate(const char* filename) {
    ...
    while (!glfwWindowShouldClose(window)) {
        if (joint_targets[0] < MAX_ANGLE_DEG * DEG_TO_RAD && joint_targets[1] < MAX_ANGLE_DEG * DEG_TO_RAD) {
            // 関節位置を1度増加
            joint_targets[0] += INCREMENT;
            joint_targets[1] += INCREMENT;
